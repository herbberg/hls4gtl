...
void charge_corr_quad(const size_t i, const ap_uint<2> charge_corr[MAX_MUON_OBJ], ap_uint<2> cc_quad[MAX_MUON_OBJ][MAX_MUON_OBJ][MAX_MUON_OBJ][MAX_MUON_OBJ])
{
...
void charge_correlation_logic(const ap_uint<2> charge_corr[MAX_MUON_OBJ], ap_uint<2> cc_double[MAX_MUON_OBJ][MAX_MUON_OBJ], ap_uint<2> cc_triple[MAX_MUON_OBJ][MAX_MUON_OBJ][MAX_MUON_OBJ], ap_uint<2> cc_quad[MAX_MUON_OBJ][MAX_MUON_OBJ][MAX_MUON_OBJ][MAX_MUON_OBJ])
{
...
#pragma HLS UNROLL
                if (charge_corr[i][1] and charge_corr[j][1] and charge_corr[k][1]) // like sign
                {
                    ...
                    {
                        cc_triple[i][j][k] = 2; // opposite sign
                    }
                }
                else
                {
...
// charge correlation quad
    ...
    {
#pragma HLS UNROLL
        charge_corr_quad(i, charge_corr, cc_quad);
    }
}
