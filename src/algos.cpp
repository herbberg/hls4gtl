...
#pragma HLS unroll
    ...
    ap_uint<1> cond_matrix[n_cond][MAX_REQ][MAX_OBJ];
...
// requirements versus objects matrix
    ...
    ap_uint<1> cond_and_or[n_cond][MAX_REQ][MAX_OBJ];
    ap_uint<1> cond_out[n_cond];
    ap_uint<1> cond_out_2_algo[n_cond];

    volatile ap_uint<1> temp_matrix[n_cond][MAX_REQ][MAX_OBJ];
    for (size_t i = 0; i < n_cond; i++)
    {
#pragma HLS unroll
        for (size_t j = 0; j < MAX_REQ; j++)
        {
#pragma HLS unroll
...
// algos
    algo = 0;
    algo |= static_cast<ap_uint<N_ALGORITHMS> > (cond_out_2_algo[0]) << 0;
    algo |= static_cast<ap_uint<N_ALGORITHMS> > (cond_out_2_algo[1]) << 1;
    algo |= static_cast<ap_uint<N_ALGORITHMS> > (cond_out_2_algo[2]) << 2;
    algo |= static_cast<ap_uint<N_ALGORITHMS> > (cond_out_2_algo[3]) << 3;
    algo |= static_cast<ap_uint<N_ALGORITHMS> > (cond_out_2_algo[4]) << 4;
}
