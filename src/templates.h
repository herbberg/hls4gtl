...
#pragma HLS ARRAY_PARTITION variable=objects complete dim=0
    ...
    comp_req_vs_obj<ap_uint<1>, T2, T3, NREQ, SLICE_MIN, SLICE_MAX>(matrix, requirements, objects);

    for (size_t i = SLICE_MIN; i <= SLICE_MAX; i++)
    {
#pragma HLS unroll
        result |= comb_cond_partial<SLICE_MIN, SLICE_MAX>(i, matrix);
    }
...
{
    return requirement.comp(external);
}
